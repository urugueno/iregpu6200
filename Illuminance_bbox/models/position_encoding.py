...
class PositionEmbeddingSine1D(nn.Module):
    """
    This is a 1D version of the position embedding, suitable for flattened sequences.
    It fits into the DETR architecture by taking a NestedTensor and returning
    a positional encoding of shape [B, C, H, W].
    """
...
class PositionEmbeddingFromCoords(nn.Module):
    """
    センサーの物理座標 (B, N, 2) から直接Positional Encodingを生成する
    """
...
class PositionEmbeddingTime1D(nn.Module):
    """
    Generates 1D positional encoding for a time sequence.
    The forward method takes a tensor of shape [B, L, C] and returns
    a positional encoding of the same shape.
    (B: batch, L: sequence length, C: hidden_dim)
    """
